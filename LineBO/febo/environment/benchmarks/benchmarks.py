...
class BenchmarkEnvironmentConfig(EnvironmentConfig):
    ...
    upper_bound_constraint = ConfigField(None, field_type=float, allow_none=True)
    noise_function = ConfigField(0.5)
    noise_obs_mode = EnumConfigField('full', enum_cls=NoiseObsMode, comment='Can be set to "full", "evaluation" or "hidden".')
...
class BenchmarkEnvironment(NoiseObsMixin, ConstraintsMixin, Environment):
    ...
    def __init__(self, path=None):
        ...
        self._upper_bound_constraint = self.config.upper_bound_constraint
        self._num_constraints = len(self._s) + int(self._upper_bound_constraint is not None) #+ int(self._lower_bound_objective is not None)
    ...
    def _get_dtype_fields(self):
        return super()._get_dtype_fields() + [('y_exact', 'f8'), ('y_max', 'f8')]#, ('z_exact', 'f8'), ('z_max', 'f8')]
